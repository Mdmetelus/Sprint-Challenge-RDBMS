...
router.get('/', (req, res) => {
  db('projects')
    ...
    .catch(err => res.status(500)
      .json({ message: "error getting data"})
    )
});
...
router.post('/', (req, res) => {
  ...
  if(!name.length || !description.length || complete === undefined) {
    res.status(400).json({ message: "Error must provide a name, a description, and "})
  } else {
  ...
  db('projects')
    ...
    .then(project => {
      if(project.length) {
        ...
        .catch(err => res.status(404).json({ message: "Error, no actions found for that project"}))
      } else { res.status(404).json({ message: "Error, no project found by that id"})}
    }
    )
    .catch(err => res.status(500).json({ message: "Error, there was an error retrieving data"}))
})
